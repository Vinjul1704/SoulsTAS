...
pub type CalculateDeltatime = unsafe extern "thiscall" fn(
    deltatime_data: *mut DeltatimeData,
    suppress_update_time: bool,
) -> f32;
...
pub fn init_darksouls2() {
    unsafe {
        ...
        let fn_frame_advance_address = process
            .scan_abs(
                "frame_advance",
                "8b ce e8 ? ? ? ? 80 be e2 00 00 00 00 74 17 8b 16",
                0,
                Vec::new(),
            )
            .unwrap()
        ...
        FRAME_ADVANCE_HOOK = Some(
            Hooker::new(
                fn_frame_advance_address,
                HookType::JmpBack(frame_advance),
                CallbackOption::None,
                0,
                HookFlags::empty(),
            )
        ...
        let fn_fps_address = if (GLOBAL_VERSION
            > Version {
                major: 0,
                minor: 0,
                build: 0,
            ...
            process
                .scan_abs(
                    "fps",
                    "55 8b ec 83 ec 3c 53 56 8b f1 8b 46 24",
                    0,
                    Vec::new(),
                )
                .unwrap()
            ...
            process
                .scan_abs(
                    "fps",
                    "55 8b ec 83 ec 3c 53 56 57 8b f1 8b 5e 20 8b 7e 24 8d 45 e4 50",
                    0,
                    Vec::new(),
                )
                .unwrap()
...
// FPS patch
pub unsafe extern "cdecl" fn fps(
    ...
    unsafe {
        if DS2_FPS_PATCH_ENABLED {
            ...
            let deltatime_override =
                if custom_deltatime < deltatime_max_stock || DS2_FPS_CUSTOM_LIMIT == 0.0 {
                    deltatime_max_stock
            ...
            match (*deltatime_data).current_deltatime_index {
                // I know this sucks, it's 1 AM and I'm about to fall over into a coma, okay?
                1 => {
            ...
            if let Some(last_frame_timestamp) = LAST_FRAME_TIMESTAMP {
                let next_frame_timestamp =
                    last_frame_timestamp + Duration::from_secs_f32(deltatime_override);
                let sleep_duration_option =
                    next_frame_timestamp.checked_duration_since(Instant::now());
...
pub unsafe extern "cdecl" fn xinput_fn(
    registers: *mut Registers,
    orig_func_ptr: usize,
    _: usize,
) -> usize {
