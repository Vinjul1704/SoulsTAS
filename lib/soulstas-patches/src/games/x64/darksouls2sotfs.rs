...
pub type CalculateDeltatime =
    unsafe extern "win64" fn(deltatime_data: *mut DeltatimeData, suppress_update_time: bool) -> f32;
...
pub fn init_darksouls2sotfs() {
    unsafe {
        ...
        let fn_fps_address = process
            .scan_abs(
                "fps",
                "48 8b c4 56 57 41 56 48 81 ec 90 00 00 00 0f 29 70 c8",
                0,
                Vec::new(),
            )
            .unwrap()
...
// FPS patch
pub unsafe extern "win64" fn fps(
    ...
    unsafe {
        if DS2SOTFS_FPS_PATCH_ENABLED {
            ...
            let deltatime_override =
                if custom_deltatime < deltatime_max_stock || DS2SOTFS_FPS_CUSTOM_LIMIT == 0.0 {
                    deltatime_max_stock
