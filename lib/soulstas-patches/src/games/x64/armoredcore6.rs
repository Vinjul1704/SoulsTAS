...
#[unsafe(no_mangle)]
#[used]
pub static mut AC6_FRAME_ADVANCE_ENABLED: bool = false;

#[unsafe(no_mangle)]
#[used]
pub static mut AC6_FRAME_RUNNING: bool = false;


#[unsafe(no_mangle)]
#[used]
pub static mut AC6_FPS_PATCH_ENABLED: bool = false;

#[unsafe(no_mangle)]
#[used]
pub static mut AC6_FPS_CUSTOM_LIMIT: f32 = 0.0f32;
...
pub fn init_armoredcore6()
{
    unsafe
    {
        ...
        let fn_frame_advance_address = process.scan_abs("frame_advance", "e8 ? ? ? ? 84 c0 74 4b 0f 1f 00", 12, Vec::new()).unwrap().get_base_address();
        ...
        let fn_fps_address = process.scan_abs("fps", "8b 83 64 02 00 00 89 83 d4 02 00 00 f3 0f 58 93 64 02 00 00", 0, Vec::new()).unwrap().get_base_address();
        ...
        let fn_fps_history_address = process.scan_abs("fps history", "48 89 01 0f b6 83 94 02 00 00 89 41 08", 0, Vec::new()).unwrap().get_base_address();
        ...
        let fn_fps_custom_limit_address = process.scan_abs("fps custom limit", "45 0f 57 d2 44 38 a3 ed 02 00 00", 0, Vec::new()).unwrap().get_base_address();
...
unsafe extern "win64" fn frame_advance(_registers: *mut Registers, _:usize)
{
    unsafe
    {
        if AC6_FRAME_ADVANCE_ENABLED
        {
            AC6_FRAME_RUNNING = false;

            while !AC6_FRAME_RUNNING && AC6_FRAME_ADVANCE_ENABLED {
...
unsafe extern "win64" fn fps_custom_limit(registers: *mut Registers, _:usize)
{
    unsafe
    {
        if AC6_FPS_PATCH_ENABLED && AC6_FPS_CUSTOM_LIMIT > 0.0f32
        {
            let ptr_flipper = (*registers).rbx as *const u8; // Flipper struct - Contains all the stuff we need
            ...
            let target_frame_delta = std::ptr::read_volatile(ptr_target_frame_delta);
            let custom_target_frame_delta = 1.0f32 / AC6_FPS_CUSTOM_LIMIT;
